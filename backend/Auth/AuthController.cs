...
namespace backend.Auth
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<ResponseDTO<string>>> ForgotPassword([FromBody] ForgotPasswordRequest request)
        {
            ...
            var otp = UtilityHelper.GenerateOtp();
            var otpHash = UtilityHelper.HashOtp(otp);
        ...
        public async Task<ActionResult<ResponseDTO<string>>> ResetPassword([FromBody] ResetPasswordRequest request)
        {
            ...
            if (otp.AttemptCount >= otp.MaxAttempt)
            {
                return BadRequest(new ResponseDTO<string>
                {
                    Success = false,
                    Code = ResponseCode.InvalidData,
                    Message = "OTP đã bị khóa",
                    Data = null,
                });
            }

            if (!UtilityHelper.VerifyOtp(otpCode, otp.OtpCodeHash))
            {
                otp.AttemptCount++;
                await _context.SaveChangesAsync();
                return BadRequest(new ResponseDTO<string>
            ...
            otp.User.Password = _passwordHasher.HashPassword(otp.User, request.NewPassword);
            otp.IsUsed = true;
